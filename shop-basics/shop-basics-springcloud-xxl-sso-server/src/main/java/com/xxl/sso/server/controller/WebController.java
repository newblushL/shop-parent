...
public class WebController extends BaseWebController {
    ...
    public String doLogin(HttpServletRequest request,
        ...
        BaseResponse<UserOutDTO> userOutDTOData =
                memberServiceFeign.xxlSsoLogin(userLoginInpDTO);
        if (!isSuccess(userOutDTOData)) {
            redirectAttributes.addAttribute("errorMsg", userOutDTOData.getMsg());

            redirectAttributes.addAttribute(Conf.REDIRECT_URL, request.getParameter(Conf.REDIRECT_URL));
            return "redirect:/login";
        }
        UserOutDTO result = userOutDTOData.getData();

        // 1、make xxl-sso user
        XxlSsoUser xxlUser = new XxlSsoUser();
        xxlUser.setUserid(String.valueOf(result.getUserId()));
        xxlUser.setUsername(result.getUserName());
